% This function calculates the the linear path in the operational space
% using a quintic polynomial
function [p_s,dp_s,ddp_s] = trajPlan2(pi,pf,ti,tf,Ts)
    syms a0 a1 a2 a3 a4 a5 t
    eq1 = subs(a0 + a1*t + a2*t^2 + a3*t^3 + a4*t^4 + a5*t^5,t,ti) == 0;
    eq2 = subs(a0 + a1*t + a2*t^2 + a3*t^3 + a4*t^4 + a5*t^5,t,tf) == norm(pf-pi);
